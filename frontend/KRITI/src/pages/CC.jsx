...
// const CC_TOKEN_ADDRESS = "0x59b670e9fA9D0A427751Af201D676719a970857b";
// const CC_MARKET_ADDRESS = "0x4ed7c70F96B99c776995fB64377f0d4aB3B0e1C1";
...
//   const [newAuction, setNewAuction] = useState({ amount: "", basePrice: "", duration: "" });
...
//       const tx = await ccMarket.placeBid(auctionId, { value: bidValue });
//       await tx.wait();
...
//           <div className="cc-balance">Your Token Balance: {tokenBalance} CC</div>
...
//                       onChange={(e) => handleBidInput(auction.id, e.target.value)}
//                     />
//                     <button
//                       className="cc-primary-button"
...
//                   </td>
//                   <td>
//                     {auction.seller.toLowerCase() === account?.toLowerCase() && !auction.isFinalized && (
//                       <button
...
const CC = () => {
  ...
  const createAuction = async () => {
    ...
    try {
      ...
      const amount = ethers.BigNumber.from(newAuction.amount);
      let basePrice = ethers.BigNumber.from(newAuction.basePrice);
      const duration = parseInt(newAuction.duration) * 86400;

      basePrice = basePrice.mul(1000000000);
  ...
  const placeBid = async (auctionId) => {
    ...
    try {
      setLoading(true);
      setError("");
      let bidValue = ethers.BigNumber.from(bidAmounts[auctionId]);

      bidValue = bidValue.mul(1000000000);
      ...
      let currentHighestBid = ethers.BigNumber.from(auction.highestBid);
      ...
      if (bidValue.lte(currentHighestBid*1000000000) || bidValue.lte(auctionBasePrice)) {
        alert("Your bid must be greater than both the current highest bid and the base price.");
        setLoading(false);
        return;
      }

      // Place the bid.
      bidValue = bidValue.mul(101).div(100);
      const tx = await ccMarket.placeBid(auctionId, { value: bidValue}); //ye abhi kiya hain 1.01
      await tx.wait();
          ...
          <table className="cc-auction-table">
            <thead>
              <tr>
                <th>ID</th>
                <th>Amount </th>
                <th>Base Price (Gwei)</th>
                <th>Highest Bid (Gwei)</th>
                <th>End Time</th>
                <th>Bid</th>
                <th>Actions</th>
              </tr>
            ...
            <tbody>
              {auctions.map((auction) => (
                <tr key={auction.id}>
                  <td>{auction.id}</td>
                  <td>{auction.tokenAmount}</td>
                  <td>{auction.basePrice/1000000000}</td>
                  <td>{auction.highestBid/1000000000}</td>
                  <td>{new Date(auction.endTime * 1000).toLocaleString()}</td>
                  <td>
                    <input
                      type="text"
